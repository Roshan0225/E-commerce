...
def consumer_reg(request):
    if request.method == "POST":
        ...
        sender_email = "roshanshankar025@gmail.com"
        Password = "wqie tdjt gcfi trir"
        reciver_email = email
...
login_required(login_url="/con_login/")
def view(request):
...
def delete(request,id):
    ...
    # for sender login
    server.login(sender_email, Password)        
    server.sendmail(sender_email, reciver_email, f""" 
    Hi {person.Name},
    Welcome !!
...
Hi {hum.Name},
    ...
    Your invoice is sent to email. 
    Any doubts contactus through given email and contact number in invoice.
    Purchase Again And Gain More Rewards.
